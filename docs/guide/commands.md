...
| `onessh log` | Show recent audit entries (`--last`, `--action`, `--alias`, `--format`); rotation: `--audit-log-max-size-mb`, `--audit-log-max-backups`, `--audit-log-max-age`, `--audit-log-compress`; subcommands `enable` / `disable` / `status` |
...
- `--data <path>` — override the data directory (default `~/.config/onessh/data`, env `ONESSH_DATA`).
- `--cache-ttl <duration>` / `--no-cache` — master password cache controls.
- `--agent-socket <path>` / `--agent-capability <token>` — memory cache agent IPC.
- `-q, --quiet` — suppress non-essential output.
- `--log` — enable audit logging for this command run.
